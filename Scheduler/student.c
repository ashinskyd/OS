...
pcb_t *static_priority_force_run_process = NULL;
...
extern void wake_up(pcb_t *process) {
    ...
    if (alg == FIFO || alg == RoundRobin)
    {
        ...
        int sleepingProcessPriority = process->static_priority;
        int lowestRunningPriority = 10000;
        int lowestPriorityProcessCpuId = 10000;
        
        int emptyProcesserId = -1;
        //find the current lowest running priority

        for (int i=0; i<cpu_count; i++){
            pcb_t *runningProcess = current[i];
            if (runningProcess != NULL){
                if (runningProcess->static_priority < lowestRunningPriority){
                    lowestRunningPriority = current[i]->static_priority;
                    lowestPriorityProcessCpuId = i;
                }
            }else{
                emptyProcesserId = i;
                break;
        ...
        //preempt the lowest running priority if the woken process priority > running priority
       
        if (emptyProcesserId != -1){
            pthread_mutex_unlock(&current_mutex);
            addReadyProcess(process);
        }
        else if (process->static_priority > lowestRunningPriority)
        {
            ...
            //if the woken process's priority is < all running priorities, just add it to the wait queue
            pthread_mutex_unlock(&current_mutex);
            addReadyProcess(process);
        }
    }
...
static pcb_t* getReadyProcess(void) {
  ...
  // ensure no other process can access ready list while we update it
    pthread_mutex_lock(&ready_mutex);
    if (alg == StaticPriority && static_priority_force_run_process != NULL){
